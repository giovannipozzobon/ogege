`wire_8 and_a_src; assign and_a_src = `A & `DATA;
...
`wire_32 and_ea_src; assign and_ea_src = `eA & `eDATA;
...
`wire_8 and_not_a_src; assign and_not_a_src = ~`A & `DATA;
...
`wire_32 and_not_ea_src; assign and_not_ea_src = ~`eA & `eDATA;
...
`wire_8 and_a_var; assign and_a_var = `A & wire_data_byte_0;
...
`wire_32 and_ea_var; assign and_ea_var = `eA & wire_data_word;
...
`wire_8 and_not_a_var; assign and_not_a_var = ~`A & wire_data_byte_0;
...
`wire_32 and_not_ea_var; assign and_not_ea_var = ~`eA & wire_data_word;
